...
void uploadFeatures(AudioFeatures features) {
    ...
    if (code > 0) {
        Serial.println("✅ FEATURES UPLOAD SUCCESS: " + String(code));
        Serial.println("Data size: " + String(jsonString.length()) + " bytes");
    } else {
        Serial.println("❌ FEATURES UPLOAD FAILED: " + String(code));
    }
...
void setup() {
    ...
    Serial.println("=== 24시간 무인 아이스크림 가게 모니터링 시스템 ===");
    Serial.println("Device: " + deviceID);
    Serial.println("특징 기반 데이터 전송 모드");
    Serial.println("압축기 On/Off 시계열 분석");
