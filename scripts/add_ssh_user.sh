#!/bin/bash
# EC2 서버에 SSH 공개키 추가 스크립트
# 사용법: ./add_ssh_user.sh "ssh-rsa  juyoung@signalcraft"

echo "🔑 EC2 서버에 SSH 공개키 추가 중..."
echo ""

# 홈 디렉토리의 .ssh 디렉토리 확인 및 생성
SSH_DIR="$HOME/.ssh"
AUTHORIZED_KEYS="$SSH_DIR/authorized_keys"

if [ ! -d "$SSH_DIR" ]; then
    echo "📁 .ssh 디렉토리 생성 중..."
    mkdir -p "$SSH_DIR"
    chmod 700 "$SSH_DIR"
    echo "✅ .ssh 디렉토리 생성 완료"
fi

# authorized_keys 파일이 없으면 생성
if [ ! -f "$AUTHORIZED_KEYS" ]; then
    echo "📄 authorized_keys 파일 생성 중..."
    touch "$AUTHORIZED_KEYS"
    chmod 600 "$AUTHORIZED_KEYS"
    echo "✅ authorized_keys 파일 생성 완료"
fi

# 키 파라미터 확인
if [ -z "$1" ]; then
    echo "❌ 오류: SSH 공개키를 인자로 제공해야 합니다."
    echo "사용법: $0 \"ssh-rsa AAAAB...\""
    exit 1
fi

PUBLIC_KEY="$1"

# 키가 이미 존재하는지 확인
if grep -q "$PUBLIC_KEY" "$AUTHORIZED_KEYS" 2>/dev/null; then
    echo "⚠️  경고: 해당 SSH 공개키가 이미 authorized_keys에 존재합니다."
    echo "추가 작업을 건너뜁니다."
    exit 0
fi

# 키 추가
echo "🔐 SSH 공개키 추가 중..."
echo "$PUBLIC_KEY" >> "$AUTHORIZED_KEYS"

# 권한 확인 및 수정
chmod 600 "$AUTHORIZED_KEYS"
chmod 700 "$SSH_DIR"

# 추가 확인
if grep -q "$PUBLIC_KEY" "$AUTHORIZED_KEYS" 2>/dev/null; then
    echo "✅ SSH 공개키가 성공적으로 추가되었습니다!"
    echo ""
    echo "📋 추가된 키 정보:"
    echo "$PUBLIC_KEY"
    echo ""
    echo "🔍 authorized_keys 파일 확인:"
    wc -l "$AUTHORIZED_KEYS"
    echo ""
    echo "✨ 김주영님(juyoung@signalcraft)의 SSH 접속 권한이 활성화되었습니다!"
else
    echo "❌ 오류: SSH 공개키 추가에 실패했습니다."
    exit 1
fi

